...
class TagStore {
    std::unordered_map<Tag, std::vector<uint16_t>> tagToEntities;
    std::unordered_map<uint16_t, std::vector<Tag>> entityToTags;

public:
    void add(uint16_t id, Tag tag) {
        tagToEntities[tag].push_back(id);
        entityToTags[id].push_back(tag);
    }

    void remove(uint16_t id, Tag tag) {
    ...
    uint16_t& getEntity(Tag tag) {
        return tagToEntities[tag][0];
    }
    // get all entities with a tag
    std::vector<uint16_t>& getEntities(Tag tag) {
        return tagToEntities[tag];
    }

    // get all tags on an entity
    std::vector<Tag>& getTags(uint16_t id) {
        return entityToTags[id];
    }

    bool has(uint16_t id, Tag tag) {
